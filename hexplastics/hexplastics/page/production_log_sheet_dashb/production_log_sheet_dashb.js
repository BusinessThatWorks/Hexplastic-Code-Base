frappe.pages["production-log-sheet-dashb"].on_page_load = function (wrapper) {
	var page = frappe.ui.make_app_page({
		parent: wrapper,
		title: "Production Log Sheet Dashboard",
		single_column: true,
	});
	...
	new ProductionLogSheetDashboard(page);
};

class ProductionLogSheetDashboard {
	constructor(page) {
		this.page = page;
	...
	setup_styles() {
		...
		style.id = "production-log-sheet-dashboard-styles";
		style.textContent = `
		...
		if (!document.getElementById("production-log-sheet-dashboard-styles")) {
	...
	bind_events() {
		...
		setTimeout(() => {
			...
			this.wrapper.on("click", "#export-logbook-btn", function (e) {
				e.stopPropagation();
				self.show_table_export_menu($(this), "entries-table", "Production_Log_Sheet");
			});
	...
	load_filter_options() {
		...
		frappe.call({
			method: "hexplastics.api.production_log_sheet_dashboard.get_filter_options",
			async: true,
			callback: function (r) {
				if (r.message) {
	...
	switch_tab(tabId) {
		...
		// No special handling needed for actual-vs-planned tab - data is already loaded
	}
	...
	refresh_data() {
		...
		frappe.call({
			method: "hexplastics.api.production_log_sheet_dashboard.get_dashboard_data",
			args: filters,
			callback: function (r) {
				if (r.message) {
	...
	show_loading() {
		...
		// Show loading for actual vs planned table
		const actualVsPlannedLoading = document.getElementById("actual-vs-planned-loading");
		if (actualVsPlannedLoading) actualVsPlannedLoading.style.display = "flex";
	}
	...
	hide_loading() {
		...
		// Hide loading for actual vs planned table
		const actualVsPlannedLoading = document.getElementById("actual-vs-planned-loading");
	...
	update_entries(entries) {
		...
		tbody.innerHTML = entries
            ...
            <tr>
                <td>
                    <a href="/app/production-log-sheet/${entry.production_log_book_id}" 
                       class="entry-link"
                       target="_blank">
	...
	update_actual_vs_planned(data) {
		if (!data) return;

		const tbody = document.getElementById("actual-vs-planned-tbody");
		const noDataMsg = document.getElementById("no-actual-vs-planned-message");
		const table = document.getElementById("actual-vs-planned-table");
		const loading = document.getElementById("actual-vs-planned-loading");
				...
				canvas.toBlob(
					(blob) => {
						if (blob) {
							...
							link.download = `Production_Sheet_Dashboard_Chart_${this.get_date_string()}.png`;
							link.href = url;
	...
	generate_pdf_with_jspdf(tableData, filePrefix, btn) {
		...
		try {
			...
			doc.text("Production Log Sheet Dashboard", 148, 15, { align: "center" });
