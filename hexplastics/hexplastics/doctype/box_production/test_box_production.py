...
class TestBoxProduction(FrappeTestCase):
	def test_apply_derived_quantities(self):
		...
		self.assertEqual(doc.transfer_to_clean_sheet, 5)
		# 40 + 30 - (5 + 2 + 3 + 1) = 59
		...
		self.assertEqual(doc.received_from_taping_dept, 80)
		self.assertEqual(doc.total_box_produced, 80)
	...
	def test_apply_closing_balances(self):
		doc = frappe._dict(
			...
			tape_checked=2996,
			received_from_taping_dept=2996,
			sticker_checked=2996,
		)
		...
		self.assertEqual(doc.sticker_available_in_plant, 0)
	...
	def test_apply_opening_balances(self):
		...
		self.assertEqual(doc.sticker_available_in_plant, 3)
	...
	def test_submit_carry_forward_chain_keeps_opening_on_prior_doc(self):
		...
		self.assertEqual(new_doc.sticker_available_in_plant, 4)  # 2 + 16 - 14
	...
	def test_create_submit_stock_entries(self, mock_get_doc, mock_get_company, _mock_nowtime):
		...
		self.assertEqual(manufacture_payload["items"][1]["qty"], 90)
