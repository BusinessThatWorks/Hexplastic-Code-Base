...
class RecycleMachineDailyProduction(Document):
    ...
    def on_submit(self):
        ...
        for row in self.production_details:
            if (
                row.item_code
                and row.grinding_mip_source_warehouse
                and flt(row.material_consumed or 0) > 0
            ):
                has_valid_source = True
            if (
                row.pp_mip_item
                and row.pp_mip_target_warehouse
                and flt(row.pp_mip_production or 0) > 0
            ):
        ...
        try:
            ...
            for row in self.production_details:
                _add_items_from_production_row(
...
def _add_items_from_production_row(
