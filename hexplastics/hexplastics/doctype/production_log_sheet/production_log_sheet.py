...
class ProductionLogSheet(Document):
	...
	def on_submit(self):
		...
		existing_stock_entry = frappe.db.exists(
			"Stock Entry",
			{"production_log_sheet": self.name, "docstatus": ["!=", 2]}
		)
		...
		if not any(d.consumption and d.consumption > 0 for d in self.raw_material_consumption):
			frappe.throw(
		...
		has_production_details = (
			...
			and any(d.manufactured_qty and d.manufactured_qty > 0 for d in self.production_details)
		)
		...
		try:
			...
			if self.manufacturing_item and self.manufactured_qty and self.manufactured_qty > 0:
				if self.manufacturing_item not in added_finished_items:
					...
					if not default_fg_warehouse:
						...
						default_fg_warehouse = frappe.db.get_value(
							"Item", self.manufacturing_item, "default_warehouse"
						) or "Finished Good - Hex"

					# Get UOM for manufacturing item
					item_uom = frappe.db.get_value("Item", self.manufacturing_item, "stock_uom")
					if not item_uom:
						frappe.throw(f"Stock UOM not found for manufacturing item {self.manufacturing_item}.")
			...
			if not stock_entry.items:
				frappe.throw("No valid Stock Entry rows were generated from this Production Log Sheet.")
			...
			frappe.throw(f"Failed to create Manufacture Stock Entry for Production Log Sheet: {frappe.utils.cstr(e)}")
